...
def delta(n_sim, n_step, alpha, theta, phi, rho, s_t, sigma_t, t, T):
    ...
    mc_s_up, mc_p_up = mc.mc_df (n_sim, n_step_left, alpha, theta, phi, rho, s_t + 1, sigma_t, T-t)
    mc_s_down, mc_p_down = mc.mc_df (n_sim, n_step_left, alpha, theta, phi, rho, s_t - 1, sigma_t, T-t)
    r = const.r
...
def vega(n_sim, n_step, alpha, theta, phi, rho, s_t, sigma_t, t, T):
    n_step_left = math.floor((T-t) * n_step / T)
    mc_s_up, mc_p_up = mc.mc_df (n_sim, n_step_left, alpha, theta, phi, rho, s_t, sigma_t + 0.01, T-t)
    mc_s_down, mc_p_down = mc.mc_df (n_sim, n_step_left, alpha, theta, phi, rho, s_t, sigma_t - 0.01, T-t)
    r = const.r
    res = math.exp(-r*(T-t)) * np.sum(mc_p_up) - math.exp(-r*(T-t)) * np.sum(mc_p_down)
    print(math.exp(-r*(T-t)) * np.sum(mc_p_up), math.exp(-r*(T-t)) * np.sum(mc_p_down))
    return res/2
